...
export class YatzyGame {
  ...
  constructor(
    ...
    this.scores =
      scores ?? YatzyGame.initScores(players.length, this.fields.length);
  ...
  private _calculateFields(player: number): void {
    for (let f = 0; f < this.fields.length; ++f) {
      ...
      this.scores[player][f] = (
        this.fields[f] as YatzyCalculatableField
      ).calculate(this.scores[player], this.fields);
    }
  }
  ...
  static fromJSON(json: any): YatzyGame {
    return new YatzyGame(
      json.players,
      json.fields.map((f: any) => YatzyField.fromJSON(f)),
      json.fieldNames,
      json.scores,
    );
  }
