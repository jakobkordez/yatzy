...
export class YatzySinglesBonusField
  extends YatzyCalculatableField
  implements YatzyScorableField
{
  ...
  calculate(scores: (number | null)[], fields: YatzyField[]): number | null {
    ...
    const ffields = fields.filter(
      (f) => f instanceof YatzyFieldSingle,
    ) as YatzyFieldSingle[];
    const potential = scores.reduce<number>(
      (p, c, i) => p + (c ?? ffields[i].num * 5),
      0,
    );
